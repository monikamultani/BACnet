...
from PIL import ImageTk, \
  Image  # sudo apt-get install python3-pil python3-pil.imagetk
...
pyglet.font.add_file(
  os.path.join(dirname, 'subChat/font/helveticaneue/HelveticaNeue.ttf'))
pyglet.font.add_file(
  os.path.join(dirname, 'subChat/font/helveticaneue/HelveticaNeueBd.ttf'))
pyglet.font.add_file(
  os.path.join(dirname, 'subChat/font/helveticaneue/HelveticaNeueIt.ttf'))

# Set global variables
app = None  # currently active app
pickle_file_names = ['personList.pkl',
                     'username.pkl']  # use to reset user or create new one
switch = ["", "", ""]
...
class Login(Frame):
  ...
  def createWidgets(self):
    ...
    self.username_field.bind('<Return>',
                             lambda event: self.create_key(self.username.get()))
    ...
    # Button
    self.button = Button(self.login_frame, text='create key',
                         command=lambda: self.create_key(self.username.get()),
  ...
  def create_key(self, username=None):
    if username != "" and len(username) <= 16:
      ...
      pickle.dump(self.dictionary,
                  open(pickle_file_names[1], "wb"))  # save username and key
...
class DisplayFile(Frame):
  ...
  def createWidgets(self):
    ...
    # Display a pdf
      ...
      filename = str(self.file_name + "[" + str(
        datetime.datetime.now()) + '].' + self.file_ending)
...
class Chat(Frame):
  ...
  def __init__(self, master=None):
    ...
    # Set EventFactory
    x = self.chat_function.get_current_event(
        self.chat_function.get_all_feed_ids()[1])
    most_recent_event = self.chat_function.get_current_event(self.feed_id)
    # self.ecf = EventCreationTool.EventFactory(
    #   most_recent_event)  # damit wieder gleiche Id benutzt wird
    self.ecf=EventFactory(most_recent_event)

    # set self.personList:
    try:  # Try to load the Variable: if it works, we know we can load it and assign it
      x = pickle.load(open(pickle_file_names[0],
                           "rb"))  # if it fails then we know we have not populated our pkl file yet
      ...
      List = list()
      pickle.dump(List,
                  open(pickle_file_names[0], "wb"))  # create an empty object
      self.person_list = pickle.load(
        open(pickle_file_names[0], "rb"))  # loading and setting it now
  ...
  def createWidgets(self):
    ...
    self.listBox1 = Listbox(self.middleFrameChat, height=30, width=25,
    ...
    self.listBox3 = Listbox(self.tabFrame, height=23, width=25,
                            yscrollcommand=self.tabScrollbar.set,
    ...
    # Chat: actions
    self.text_field = Entry(self.bottomFrameChat, textvariable=self.msg,
                            font=('HelveticaNeue', 10))
    self.text_field.configure(bg='#f0f0f0')
    self.text_field.bind('<Return>',
                         lambda event: self.check_and_save(self.msg.get(),
                                                           self.partner[1]))
    self.text_field.bind("<Button-1>",
                         lambda event: self.clear_on_entry("text_field"))
    self.send_button = Button(self.bottomFrameChat, text='Send',
                              command=lambda: self.check_and_save(
                                self.msg.get(), self.partner[1]), bg="#25D366",
                              activebackground="#21B858",
                              font=('HelveticaNeue', 10))


    self.update_button = Button(self.bottomFrameChat, text='Update',
                                command=lambda: self.loadChat(self.partner),
                                bg="white", font=('HelveticaNeue', 10))
    self.create_Button = Button(self.tabTopBar, text="   create   ",
                                command=lambda: self.saveTypeAndSwitchState(
                                  "create"), bg="#25D366",
                                activebackground="#21B858",
                                font=('HelveticaNeue', 11))  # green
    self.join_Button = Button(self.tabTopBar,
                              text="           join             ",
                              command=lambda: self.saveTypeAndSwitchState(
                                "join"), bg="#34B7F1",
                              activebackground="#0f9bd7",
                              font=('HelveticaNeue', 11))  # blue
    self.privateChat_Button = Button(self.tabTopBar, text="Private Chat",
                                     command=lambda: self.saveTypeAndSwitchState(
                                       "private Chat"), bg="#25D366",
                                     activebackground="#21B858",
                                     font=('HelveticaNeue', 11))  # lightgreen
    self.group_Button = Button(self.tabTopBar, text="    Group    ",
                               command=lambda: self.saveTypeAndSwitchState(
                                 "group"), bg="#34B7F1",
                               activebackground="#0f9bd7",
                               font=('HelveticaNeue', 11))  # lightblue

    self.confirm_Button = Button(self.tabTopBar, text="OK",
                                 command=lambda: self.saveTypeAndSwitchState(
                                   "confirm"), bg="#25D366",
                                 activebackground="#21B858",
                                 font=('HelveticaNeue', 11))  # green
    self.back_Button = Button(self.tabTopBar, text="back",
                              command=lambda: self.saveTypeAndSwitchState(
                                "back"), bg="white", font=('HelveticaNeue', 11),
                              highlightcolor="white")  # green
    self.button_state = 0
    self.ButtonType = ""
    self.ButtonTask = ""
    self.BackTask = ""

    self.id_field = Entry(self.tabTopBar, textvariable=self.entry_field,
                          state=NORMAL)  # readonly state
    self.id_field.bind("<Button-1>",
                       lambda event: self.clear_on_entry("id_field"))
  ...
  def yview(self,
      *args):  # self configured yview command. the only job is to trigger both sync methods from abouve
    ...
    for i in range(1, len(chat)):
      ...
      if len(chat_message) == 4:
        ...
        if partner_username != self.username:  # message from the partner(s)
          ...
          if chat_type == "group":
            ...
            try:
              self.listBox1.itemconfig('end', bg='white',
                                       foreground=Colorize.name_to_color(
                                         partner_username))
            except:
          ...
          self.listBox1.insert('end', "{:<22}{:>16}".format("", time.strftime(
            "%H:%M %d.%m.%Y", time.gmtime(chat[i][1] + timezone))))
          ...
          self.listBox2.insert('end', "{:<22}{:>16}".format("", time.strftime(
            "%H:%M %d.%m.%Y", time.gmtime(chat[i][1] + timezone))))
  ...
  def loadChat(self,
    ...
    chat_type = \
    self.chat_function.get_full_chat(self.partner[1])[0][0].split("#split:#")[
      3]  # get the type of the chat (private or group)
  ...
  def check_for_new_messages(self, person_nr):
    length = len(
      self.chat_function.get_chat_since(self.person_list[person_nr][2],
                                        self.person_list[person_nr][1]))
    unread_messages = length - self.count_users_in_chat(
        self.person_list[person_nr][1], self.person_list[person_nr][2])
  ...
  def addPartners(self):
    if len(self.person_list) != 0:
      ...
      for i in range(len(self.person_list)):
        ...
        self.listBox3.insert('end', TextWrapper.mergeNameCounter(
            self.person_list[i][0], counter))
  ...
  def switchState(self,
      Type=None):  # creates a menu like behaviour of the buttons
    # STATE = 0
    if self.button_state == 0:  # after first click (we now know the Button Type ('create' or 'join'))
      ...
      self.back_Button.grid(row=0, column=3,
                            sticky="e")  # add back button for second state
    ...
    # STATE = 2
      ...
      if self.BackTask != 'back':
        if self.ButtonType == 'join':
          ...
          if len(
              self.id_field.get()) <= 16 and self.id_field.get() != "" and self.id_field.get() != " ":
  ...
  def check_and_save(self, message, chat_id):
    if self.partner[1] != "":
      ...
      if message == "" or count == 0 or not self.person_list or not self.partner:
        ...
        if os.path.isdir(file_path[0:file_path.rfind(
            ...
            if length > 100 * kilobyte:  # file too big
              ...
              for i in range(partition_len):
                if i == int(partition_len) - 1:  # We arrived at last part
                  self.save(
                    file_string[i * kilobyte:] + "#split:#" + file_type + str(
                      partition_len) + "_" + file_name, chat_id)
                else:
                  self.save(file_string[i * kilobyte:(
                                                           i + 1) * kilobyte] + "#split:#filepart#split:#filepart",
                            chat_id)

            else:  # length <= 1000
              self.save(file_string + "#split:#" + file_type + file_name,
                        chat_id)
          else:  # file not found
            self.text_field.delete(0, 'end')
            self.text_field.insert(0,
                                   "Sorry, file does not exist, please try again!")
        else:  # directory not found
          self.text_field.delete(0, 'end')
          self.text_field.insert(0,
                                 "Sorry, given path does not exist, please try again!")
  ...
  def save(self, message, chatID):
    ...
    new_event = self.ecf.next_event('chat/saveMessage',
                                    {'messagekey': to_save, 'chat_id': chatID,
                                     'timestampkey': time.time()})
    self.chat_function.insert_chat_msg(new_event)

    if self.partner[1] != "":
      self.loadChat(
        self.partner)  # updating chat, so the send message is also added in the listbox
    self.text_field.delete(0, 'end')
  ...
  def create_chat(self, ID, name=None):
    if not name:
      ...
      self.person_list.append(
          [name, ID, 0])  # it's a group so we do know the name
      self.partner[0] = name
      self.partner[1] = ID
  ...
  def join_chat(self, ID):
    if self.is_joinable(ID):
      self.person_list.append([ID, ID, 0])
      partnerName = \
      self.chat_function.get_full_chat(ID)[0][0].split("#split:#")[
        1]  # taking the name of the partner for the frome the first message
      self.person_list[-1][
        0] = partnerName  # index -1 is the index of the last list-element
  ...
  def is_joinable(self, chatID):
    if len(chatID) != 6:
      ...
      if chat[0][0].split("#split:#")[
        3] == "private" and self.count_users_in_chat(
          chatID) == 2:  # max. member for private chat: 2
        return False

      for i in range(
          len(self.person_list)):  # So the person cannot write with himself
  ...
  def open_file1(self):
    ...
    if selection or selection == 0:
      ...
      if len(item) >= 21 and (item[0:19] == "Click to open PDF. " or item[
        ...
        index = int(
            item[item.find("(") + 1:item.find(")")])  # getting the index
        ...
        self.open_file(assembled_content,
                       name_of_file[name_of_file.find("_") + 1:], type_of_file)
  ...
  def generate_ID():  # generates a secure random id
    ID = list(''.join(random.SystemRandom().choice(
      string.ascii_lowercase + string.ascii_uppercase + string.digits) for i in
                      range(6)))
...
try:  # Try to get the key (if it works, then  we can skip the login and go directly to the chat)
  ...
  print(
    "Key has NOT been found: You are redirected to 'Login' to create a User Profile first")
  # test stays 'name' or ""
...
if probe == 'no_key':  # this is only the case when there
  ...
  app = Chat(
    master=root)  # If there is already an exiting key, we can just login
...
pickle.dump(app.person_list,
            open(pickle_file_names[0], "wb"))  # create an empty object
print("\"personList\" has been saved to \"" + pickle_file_names[
  0] + "\": personList = ", app.person_list)
